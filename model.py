...
def OnBalanceVolume(price_volume_series):
    price_volume_series["Daily_Price_Change"] = price_volume_series[
        "Smoothed_adj_close"
    ].diff()
    price_volume_series["Direction"] = price_volume_series["Daily_Price_Change"].apply(
...
def StochasticOscillator(
    price_low_high_series: pd.DataFrame, K_high: int = 14, K_low: int = 3
):
    price_low_high_series["L14"] = (
        price_low_high_series["Low"].rolling(window=K_high).min()
    )
    price_low_high_series["H14"] = (
        price_low_high_series["High"].rolling(window=K_high).max()
    )
    price_low_high_series["%K"] = 100 * (
        (price_low_high_series["Smoothed_adj_close"] - price_low_high_series["L14"])
        / (price_low_high_series["H14"] - price_low_high_series["L14"])
    )
    price_low_high_series["%D"] = (
        price_low_high_series["%K"].rolling(window=K_low).mean()
    )
...
def MACD(
    price_series: [pd.DataFrame, list],
    short_period: int = 12,
    long_period: int = 26,
    signal_period: int = 9,
):
    MA_Fast = (
        price_series["Smoothed_adj_close"].ewm(span=12, min_periods=long_period).mean()
    )
...
def categorical_price(price_series: [pd.DataFrame, list], forecast_period: int):
    price = np.sign(
        np.log(
            price_series["Smoothed_adj_close"].shift(-forecast_period)
            / price_series["Smoothed_adj_close"]
        )
    )
